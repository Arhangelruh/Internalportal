...
namespace InternalPortal.Web.Controllers
{
	...
	{
		private readonly ConfigurationTest _configurationTest = configurationTest.Value ?? throw new ArgumentNullException(nameof(configurationTest));
		...
		private readonly ITestScoreService _testScoreService = testScoreService ?? throw new ArgumentNullException(nameof(testScoreService));
		private readonly ITestAnswerService _testAnswerService = testAnswerService ?? throw new ArgumentNullException(nameof(testAnswerService));
		private readonly ITestQuestionService _testQuestionService = testQuestionService ?? throw new ArgumentNullException(nameof(testQuestionService));
		...
		private readonly ILogger<TestingController> _logger = logger ?? throw new ArgumentNullException(nameof(logger));

		/// <summary>
		/// Build test.
		/// </summary>
		/// <returns>TestViewModel model</returns>
		[Authorize(Roles = UserConstants.CashStudents)]
		[HttpGet]
			...
			if (score == null || score.Score < _configurationTest.Repeat)
			{
				...
				if (testData.Questions.Count > 0)
				{
					...
					foreach (var question in testData.Questions)
					{
						...
						{
							_logger.LogWarning($"При формировании теста  {profile.Name} {profile.LastName} нет ответов на вопрос: {question.QuestionText}");
						}
		...
		public async Task<IActionResult> UserResult(int testId)
		{
			...
			if (testAnswers.Count != 0)
			{
				...
				{
					_ = answer.Answers.OrderBy(a => a.Id).ToList();
				}
			}
